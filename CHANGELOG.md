...
### Adicionado
- Documentação completa em PDF (`docs/Documentacao-Portal-Provisionamento-M365.pdf`, 26 páginas): visão geral e importância do sistema, funcionalidades com telas (dados fictícios), arquitetura, segurança, permissões, implantação, operação, configurações e glossário; fonte HTML e gerador em `docs/pdf/`; link de download no README.
- Sprint 11: novo visual inspirado no Fluent 2 — menu lateral com ícones (recolhível no desktop, gaveta no celular), topo com avatar e menu da conta (Popover API), tema claro/escuro automático com botão (preferência salva só no navegador, sem piscar), página inicial com atalhos, cartões, tabelas, formulários, selos de status, linha do tempo e abas redesenhados; CSS moderno sem build nem dependências (`@layer`, aninhamento, `light-dark()`, `color-mix()`, `:has()`, view transitions, `prefers-reduced-motion`); ícones SVG próprios; acessibilidade (pular para o conteúdo, foco visível, `aria-current`); tudo compatível com a CSP (nenhum estilo ou script inline). Linguagem simples nas telas: sem nomes de variáveis, siglas ou códigos técnicos ("Modo simulação", "Login", "véspera/dia da admissão", "código de acesso inicial", "Recursos do Microsoft 365"); erros do Microsoft Graph traduzidos para mensagens claras, com o detalhe técnico só no log; informações para quem administra o Azure recolhidas em "Como definir…"; guia `docs/SOLUCAO_DE_PROBLEMAS.md`.
