...
### Adicionado
- Sprint 2: login com Microsoft Entra ID via App Service Authentication usando a Managed Identity como credencial federada (sem Client Secret); App Roles Solicitante/Aprovador/Administrador atribuídos a grupos; validação de tenant, audiência e papéis no backend; modo `AUTH_MODE=dev` para desenvolvimento local; script `New-EntraApplication.ps1`.
